...
description: |
  Interactive agent that scaffolds a Networking Swift package (Network + NetworkLive products) and customizes it for the project — runs the scaffold script, then walks through BaseURL cases and any app-specific auth adapter needs.

  Use when the user wants to add a generic networking layer to an iOS project, mentions creating a Network/NetworkLive package, or needs HTTPClient/NetworkService/endpoint infrastructure.
...
2. Open `<target>/Sources/Network/Endpoint/BaseURL.swift` — show the placeholder to the user.
...
2. Ask: **"Does your app need any auth headers beyond `Authorization: Bearer {token}`?"**
   ...
   These live in `<target>/Sources/NetworkLive/Client/HTTP/`.
...
- **Match existing patterns.** If the repo already has `Network`/`NetworkLive` targets, scan them first and skip re-creating existing files.
